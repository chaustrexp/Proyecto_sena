...
class CoordinacionController extends BaseController {
    ...
    public function store() {
        if ($_SERVER['REQUEST_METHOD'] !== 'POST') {
            $this->redirect('/Gestion-sena/dashboard_sena/coordinacion');
            return;
        }
        
        // Validar campos requeridos
        $required = ['coord_descripcion', 'CENTRO_FORMACION_cent_id', 'coord_nombre_coordinador', 'coord_correo'];
        $errors = $this->validate($_POST, $required);
        
        if (!empty($errors)) {
            $_SESSION['error'] = 'Por favor complete todos los campos requeridos';
            $this->redirect('/Gestion-sena/dashboard_sena/coordinacion/crear');
            return;
        }
    ...
    public function update($id) {
        ...
        if (!empty($errors)) {
            $_SESSION['error'] = 'Por favor complete todos los campos requeridos';
            $this->redirect('/Gestion-sena/dashboard_sena/coordinacion/editar/' . $id);
            return;
        }
